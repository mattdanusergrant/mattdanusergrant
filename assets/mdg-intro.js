...
(function(){
  ...
  function makeSketches(ctx){
    ...
    SK.cards=function(w,h,t){
      var cx=w/2,cy=h/2,m=Math.min(w,h),ch=m*0.34,cw=ch*0.64,rr=cw*0.14,N=14;
      var spin=t*0.16,bloom=Math.sin(t*0.6)*0.5+0.5,r0=m*(0.006+0.03*bloom);
      ...
      for(var i=0;i<N;i++){
        var a=i/N*TAU+spin;
        ctx.save();
        ctx.translate(cx+Math.cos(a)*(r0+ch/2),cy+Math.sin(a)*(r0+ch/2));
        ctx.rotate(a-Math.PI/2);
        rrect(-cw/2,-ch/2,cw,ch,rr);
        ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fill();
        ctx.lineWidth=Math.max(1,cw*0.06);ctx.strokeStyle=col(0,0.9);ctx.stroke();
        ctx.restore();
      }
    };
